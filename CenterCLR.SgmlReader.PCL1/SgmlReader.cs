...
namespace CenterCLR.Sgml
{
	...
	public class SgmlReader : XmlReader
	{
		...
		public SgmlReader(TextReader tr)
		{
		...
		private void LazyLoadDtd(Uri baseUri)
		{
			if (this.m_dtd == null && !this.m_ignoreDtd)
			{
				...
				else
				{
					...
					{
						baseUri = new Uri(this.m_syslit);
					}
					this.m_dtd = SgmlDtd.Parse(baseUri, this.m_docType, this.m_pubid, baseUri.AbsoluteUri, this.m_subset, this.m_streamOpener, null);
				}
			}
		...
		public Func<Uri, StreamInformation> StreamOpener
		{
			get
			{
				return this.m_streamOpener;
			}
			set
			{
				this.m_streamOpener = value;
			}
		}
		...
		private void OpenInput()
		{
			...
			if (this.Href != null)
			{
				this.m_current = new Entity("#document", null, this.m_href, this.m_streamOpener);
			}
			else if (this.m_inputStream != null)
			{
				this.m_current = new Entity("#document", null, this.m_inputStream, this.m_streamOpener);
			}
			else
			{
			...
			else
			{
				...
				if (this.m_dtd != null && !string.IsNullOrEmpty(name))
				{
					...
					if (e != null)
					{
						...
						{
							Entity ex = new Entity(name, e.PublicId, e.Uri, this.m_current.StreamOpener);
