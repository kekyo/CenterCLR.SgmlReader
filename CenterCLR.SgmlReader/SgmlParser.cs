...
namespace CenterCLR.Sgml
{
	...
	/// <summary>
	/// Stream information by stream opener.
	/// </summary>
	public sealed class StreamInformation
	{
		...
		/// <summary>
		/// Default (Suggested) Encoding.
		/// </summary>
		/// <remarks>Derived may be remote transport.</remarks>
	...
	public class Entity : IDisposable
	{
		...
		private Func<Uri, StreamInformation> m_streamOpener;
		...
		public Entity(string name, string pubid, string uri, Func<Uri, StreamInformation> streamOpener)
		{
			m_name = name;
			m_publicId = pubid;
			m_uri = uri;
			m_streamOpener = streamOpener;
		...
		/// <param name="streamOpener">Stream opener delegate.</param>
		public Entity(string name, Uri baseUri, TextReader stm, Func<Uri, StreamInformation> streamOpener)
		{
			m_name = name;
			m_isInternal = true;
			m_stm = stm;
		...
		/// <summary>
		/// Get/Set Stream opener delegate.
		/// </summary>
		/// <remarks>Call delegate by parser if require external resource.</remarks>
		public Func<Uri, StreamInformation> StreamOpener
		{
			get
			{
				return m_streamOpener;
			}
		}
		...
		public void Open(Entity parent, Uri baseUri)
		{
			...
			else
			{
				...
				if (m_streamOpener == null)
				{
					this.Error("StreamOpener is not set '{0}', '{1}'", this.m_name, this.m_resolvedUri);
				}
				else
				{
					var si = m_streamOpener(m_resolvedUri);
					Stream stream = si.Stream;
					this.m_weOwnTheStream = true;
					HtmlStream html = new HtmlStream(stream, si.DefaultEncoding);
		...
		private static int ConvertToUtf32(char highSurrogate, char lowSurrogate)
		{
		...
		/// <exception cref="SgmlParseException">Always thrown.</exception>
		public void Error(string msg, params object[] arg)
		{
	...
	*/
	...
	public class SgmlDtd
	{
		...
		public static SgmlDtd Parse(Uri baseUri, string name, string pubid, string url, string subset, Func<Uri, StreamInformation> streamOpener, XmlNameTable nt)
		{
			SgmlDtd dtd = new SgmlDtd(name, nt);
			if (!string.IsNullOrEmpty(url))
			{
				dtd.PushEntity(baseUri, new Entity(dtd.Name, pubid, url, streamOpener));
			}
		...
		private void ParseEntity()
		{
			...
			else
			{
				...
				else
				{
					...
					e = new Entity(name, pubid, uri, this.m_current.StreamOpener);
				}
			}
			ch = this.m_current.SkipWhitespace();
			if (ch == '-')
